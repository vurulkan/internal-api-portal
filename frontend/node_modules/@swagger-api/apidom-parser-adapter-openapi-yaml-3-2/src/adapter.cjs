 ...
 * @public
 */
const detectionRegExp = exports.detectionRegExp = /(?<YAML>^(["']?)openapi\2\s*:\s*(["']?)(?<version_yaml>3\.2\.(?:[1-9]\d*|0))\3(?:\s+|$))|(?<JSON>"openapi"\s*:\s*"(?<version_json>3\.2\.(?:[1-9]\d*|0))")/m;
...
const parse = async (source, options = {}) => {
  ...
  if ((0, _ramdaAdjunct.isNotUndefined)(result)) {
    const openApiElement = _apidomNsOpenapi.OpenApi3_2Element.refract(result, refractorOpts);

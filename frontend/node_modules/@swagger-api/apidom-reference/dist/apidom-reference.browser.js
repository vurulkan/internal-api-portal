(function (global, factory) {
  ...
  (global = typeof globalThis !== 'undefined' ? globalThis : global || self, factory(global.apidomReference = {}));
   ...
   */
  var dropLastWhile = /*#__PURE__*/_curry2( /*#__PURE__*/_dispatchable([], _xdropLastWhile, dropLastWhile$1));
  ...
  var replaceAll$1 = function replaceAll(searchValue, replaceValue, str) {
  ...
  var replaceAllPonyfill = curryN(3, replaceAll$1);
  ...
  const isWindows = () => {
    var _a, _b;
    return /^win/.test((_b = (_a = globalThis.process) == null ? void 0 : _a.platform) != null ? _b : "");
  };
  ...
  const hasProtocol = pipe(getProtocol, isNotUndefined);
  ...
  const cwd = () => {
    ...
    const path = (_b = (_a = globalThis.process) == null ? void 0 : _a.cwd()) != null ? _b : "/";
  ...
  var __publicField$9 = (obj, key, value) => __defNormalProp$b(obj, typeof key !== "symbol" ? key + "" : key, value);
  class File {
    constructor({ uri, mediaType = "text/plain", data, parseResult }) {
      __publicField$9(this, "uri");
      __publicField$9(this, "mediaType");
      __publicField$9(this, "data");
      __publicField$9(this, "parseResult");
      this.uri = uri;
  ...
  class ReferenceSet {
    constructor({ refs = [], circular = false } = {}) {
      __publicField$8(this, "rootRef");
      __publicField$8(this, "refs");
      __publicField$8(this, "circular");
      this.refs = [];
  ...
  const baseURILens = lens(path$1(["resolve", "baseURI"]), assocPath(["resolve", "baseURI"]));
  ...
  const filter = (method, parameters, plugins) => __async$6(null, null, function* () {
    const pluginResults = yield Promise.all(plugins.map(invokeArgs([method], parameters)));
    return plugins.filter((plugin, index) => pluginResults[index]);
  });
  const run = (method, parameters, plugins) => __async$6(null, null, function* () {
    let lastError;
    for (const plugin of plugins) {
      try {
        const result = yield plugin[method].call(plugin, ...parameters);
  ...
  const getCacheFileResult = (_0) => __async$5(null, [_0], function* ({
    cacheKey,
    cacheTTL
  }) {
    if (cacheTTL === 0) {
    ...
    try {
      ...
      if (response) {
        const { cachedResult, cachedError, timestamp } = yield response.json();
        const now = Date.now();
  ...
  const setCacheFileResult = (_0) => __async$5(null, [_0], function* ({
    cacheKey,
    result,
    error,
    cacheTTL
  ...
  const readFile$1 = (file, options) => __async$5(null, null, function* () {
    var _a, _b, _c;
    const { cacheTTL = 0 } = options.resolve.resolverOpts;
    const cacheKey = `read_${file.uri}`;
    const { cachedResult, cachedError } = yield getCacheFileResult({ cacheKey, cacheTTL });
    ...
    try {
      const { result } = yield run("read", [file], resolvers);
      const stringifiedData = new File(__spreadProps$1(__spreadValues$1({}, file), { data: result })).toString();
      yield setCacheFileResult({ cacheKey, result: stringifiedData, error: null, cacheTTL });
      return result;
    } catch (error) {
      yield setCacheFileResult({
        cacheKey,
        result: null,
        error: (_c = (_b = (_a = error == null ? void 0 : error.cause) == null ? void 0 : _a.cause) != null ? _b : error == null ? void 0 : error.cause) != null ? _c : error,
        cacheTTL
      });
  ...
  const parseFile = (file, options) => __async$4(null, null, function* () {
    ...
    const parsers = yield filter("canParse", [file, options], optsBoundParsers);
    if (isEmpty(parsers)) {
      throw new UnmatchedResolverError(file.uri);
    }
    try {
      const { plugin, result } = yield run("parse", [file, options], parsers);
  ...
  const parse$1 = (uri, options) => __async$4(null, null, function* () {
    const file = new File({
      uri: sanitize(stripHash(uri)),
      mediaType: options.parse.mediaType
    });
    const data = yield readFile$1(file, options);
    return parseFile(new File(__spreadProps(__spreadValues({}, file), { data })), options);
  });
  ...
  const resolveApiDOM$1 = (element, options) => __async$3(null, null, function* () {
    let parseResult = element;
    if (!isParseResultElement(element)) {
      const elementClone = cloneShallow(element);
      elementClone.classes.push("result");
      parseResult = new ParseResult([elementClone]);
    }
    const sanitizedURI = sanitize(stripHash(options.resolve.baseURI));
    const file = new File({
      uri: sanitizedURI,
      parseResult,
      mediaType: options.parse.mediaType
    });
    const resolveStrategies = yield filter(
      "canResolve",
      [file, options],
    ...
    try {
      const { result } = yield run("resolve", [file, options], resolveStrategies);
      return result;
    } catch (error) {
      throw new ResolverError(`Error while resolving file "${file.uri}"`, { cause: error });
    }
  });
  const resolve$1 = (uri, options) => __async$3(null, null, function* () {
    const parseResult = yield parse$1(uri, options);
    const mergedOptions = merge(options, { resolve: { baseURI: sanitize(uri) } });
    return resolveApiDOM$1(parseResult, mergedOptions);
  });
  ...
  const dereferenceApiDOM$1 = (element, options) => __async$2(null, null, function* () {
    let parseResult = element;
    ...
    const dereferenceStrategies = yield filter(
      "canDereference",
      [file, options],
    ...
    try {
      const { result } = yield run("dereference", [file, options], dereferenceStrategies);
  ...
  const dereference$1 = (uri, options) => __async$2(null, null, function* () {
    const { refSet } = options.dereference;
    const sanitizedURI = sanitize(uri);
    let parseResult;
    if (refSet !== null && refSet.has(sanitizedURI)) {
      ({ value: parseResult } = refSet.find(propEq(sanitizedURI, "uri")));
    } else {
      parseResult = yield parse$1(uri, options);
    }
    ...
    return dereferenceApiDOM$1(parseResult, mergedOptions);
  });
  ...
  const bundle$1 = (uri, options) => __async$1(null, null, function* () {
    const { refSet } = options.bundle;
    const sanitizedURI = sanitize(uri);
    const mergedOptions = merge(options, { resolve: { baseURI: sanitizedURI } });
    let parseResult;
    if (refSet !== null && refSet.has(sanitizedURI)) {
      ({ value: parseResult } = refSet.find(propEq(sanitizedURI, "uri")));
    } else {
      parseResult = yield parse$1(uri, mergedOptions);
    }
    const file = new File({
    ...
    const bundleStrategies = yield filter(
      "canBundle",
      [file, mergedOptions],
    ...
    try {
      const { result } = yield run("bundle", [file, mergedOptions], bundleStrategies);
      return result;
    } catch (error) {
  ...
  class Parser {
    constructor({
       ...
       */
      __publicField$6(this, "allowEmpty");
      /**
       * Whether to generate source map during parsing.
       */
      __publicField$6(this, "sourceMap");
      /**
       * List of supported file extensions.
       */
      __publicField$6(this, "fileExtensions");
      /**
       * List of supported media types.
       */
      __publicField$6(this, "mediaTypes");
      this.name = name;
  ...
  class HTTPResolver extends Resolver {
    constructor(options) {
      ...
      __publicField$4(this, "redirects");
      __publicField$4(this, "withCredentials");
  ...
  const readFile = (_0, ..._1) => __async(null, [_0, ..._1], function* (uri, options = {}) {
    const mergedOptions = merge(defaultOptions, options);
    const file = new File({ uri: sanitize(uri) });
    return readFile$1(file, mergedOptions);
  });
  const parse = (_0, ..._1) => __async(null, [_0, ..._1], function* (uri, options = {}) {
    const mergedOptions = merge(defaultOptions, options);
    return parse$1(uri, mergedOptions);
  });
  const resolve = (_0, ..._1) => __async(null, [_0, ..._1], function* (uri, options = {}) {
    const mergedOptions = merge(defaultOptions, options);
    return resolve$1(uri, mergedOptions);
  });
  const resolveApiDOM = (_0, ..._1) => __async(null, [_0, ..._1], function* (element, options = {}) {
    const mergedOptions = merge(defaultOptions, options);
    return resolveApiDOM$1(element, mergedOptions);
  });
  const dereference = (_0, ..._1) => __async(null, [_0, ..._1], function* (uri, options = {}) {
    const mergedOptions = merge(defaultOptions, options);
    return dereference$1(uri, mergedOptions);
  });
  const dereferenceApiDOM = (_0, ..._1) => __async(null, [_0, ..._1], function* (element, options = {}) {
    const mergedOptions = merge(defaultOptions, options);
    return dereferenceApiDOM$1(element, mergedOptions);
  });
  const bundle = (_0, ..._1) => __async(null, [_0, ..._1], function* (uri, options = {}) {
    const mergedOptions = merge(defaultOptions, options);
    return bundle$1(uri, mergedOptions);
  });

  exports.BundleError = BundleError;
  exports.BundleStrategy = BundleStrategy;
  exports.DereferenceAncestorLineage = AncestorLineage;
  exports.DereferenceError = DereferenceError;
  exports.DereferenceStrategy = DereferenceStrategy;
  exports.EvaluationElementIdError = EvaluationElementIdError;
  exports.EvaluationJsonSchema$anchorError = EvaluationJsonSchema$anchorError;
  exports.EvaluationJsonSchemaUriError = EvaluationJsonSchemaUriError;
  exports.File = File;
  exports.HTTPResolver = HTTPResolver;
  exports.InvalidJsonSchema$anchorError = InvalidJsonSchema$anchorError;
  exports.JsonSchema$anchorError = JsonSchema$anchorError;
  exports.JsonSchemaURIError = JsonSchemaUriError;
  exports.MaximumBundleDepthError = MaximumBundleDepthError;
  exports.MaximumDereferenceDepthError = MaximumDereferenceDepthError;
  exports.MaximumResolveDepthError = MaximumResolveDepthError;
  ...
  exports.UnmatchedBundleStrategyError = UnmatchedBundleStrategyError;
  exports.UnmatchedDereferenceStrategyError = UnmatchedDereferenceStrategyError;
  exports.UnmatchedResolveStrategyError = UnmatchedResolveStrategyError;
  exports.UnmatchedResolverError = UnmatchedResolverError;
  exports.bundle = bundle;
  exports.dereference = dereference;
  exports.dereferenceApiDOM = dereferenceApiDOM;
  exports.mergeOptions = merge;

(function (global, factory) {
  ...
  (global = typeof globalThis !== 'undefined' ? globalThis : global || self, factory(global.apidomNsApiDesignSystems = {}));
    ...
    return {
      visitor: {
        OperationElement: {
          enter(element, ...rest) {
            ...
            operationIdentifiers.push(
              ...
              ["http", "request"],
              ["http", "request", "url"],
              ["http", "request", "url", serializer(parentPathItem.meta.get("path"))],
              ["http", "request", "method"],
              ["http", "request", "method", serializer(element.meta.get("http-method")).toLowerCase()]
            );
            (_a = parentPathItem == null ? void 0 : parentPathItem.parameters) == null ? void 0 : _a.forEach((parameter) => {
              if (isStringElement(parameter.in) && isStringElement(parameter.name) && serializer(parameter.in) === "header") {
                operationIdentifiers.push(
                  ["http", "request", "header"],
                  ["http", "request", "header", serializer(parameter.name)],
                  ["http", "message", "header"],
                  ["http", "message", "header", serializer(parameter.name)]
                );
              }
            });
          },
        ...
        ParameterElement(element) {
          if (isStringElement(element.in) && isStringElement(element.name) && serializer(element.in) === "header") {
            operationIdentifiers.push(
              ["http", "request", "header"],
              ["http", "request", "header", serializer(element.name)],
              ["http", "message", "header"],
              ["http", "message", "header", serializer(element.name)]
            );
          }
        },
        ...
        ResponseElement: {
          enter(element) {
            ...
            if (element.meta.hasKey("http-status-code")) {
              const statusCode = String(serializer(element.meta.get("http-status-code")));
    ...
    return {
      visitor: {
        OperationElement(element, ...rest) {
          ...
          httpMethod.content = serializer(httpMethod).toLowerCase();
        ...
        RequestBodyElement(element) {
          ...
          element.contentProp.forEach((mediaType, key) => {
            standardIdentifiers.push(
              {
                subject: ["http", "request", "header"],
                value: new minimExports.StringElement("Content-Type", cloneDeep(key.meta))
              },
              {
                subject: ["http", "message", "header"],
                value: new minimExports.StringElement("Content-Type", cloneDeep(key.meta))
  ...
  const visitorOptions$1 = { keyMap: keyMap$1, nodeTypeGetter: getNodeType$1 };
  const select = (openAPIElement, standardIdentifier) => {
    ...
    const visitor = {
      OperationElement(element) {
        ...
        const standardIdentifiers = serializer(element.meta.get("ads-s-standard-identifier"));
        if (includes(serializer(standardIdentifier), standardIdentifiers)) {
    ...
    visit$1(openAPIElement, visitor, visitorOptions$1);
    return selected;
  };

  const visitorOptions = { keyMap: keyMap$1, nodeTypeGetter: getNodeType$1 };
  const access = (selected, standardIdentifier) => {
    const strStandardIdentifier = String(serializer(standardIdentifier));
    const values = new minimExports.ArrayElement();
    const visitor = {
      enter(element) {
        ...
        element.meta.get("ads-a-standard-identifier").content.filter((accessorMapping) => {
          return String(serializer(accessorMapping.get("subject"))) === strStandardIdentifier;
        }).forEach((accessorMapping) => {
          values.push(accessorMapping.get("value"));
        });
      }
    };
    visit$1(selected, visitor, visitorOptions);
    return values;
  };
  ...
  const makeMessage = (value, requirementLevel, standardIdentifier) => {
    const primitiveValue = serializer(value);
    const primitiveStandardIdentifier = JSON.stringify(serializer(standardIdentifier));
    if (serializer(requirementLevel) === "may") {
  ...
  const validateValue = (value, requirement) => {
    ...
    if (serializer(requirement.level) === "may") {
      const isValid = may(serializer(value), serializer(requirement.values));
      if (!isValid) {
  ...
  const isMainElement = createPredicate(
    ({ hasBasicElementProps, isElementType, primitiveEq }) => {
      return (element) => element instanceof Main || hasBasicElementProps(element) && isElementType("main", element) && primitiveEq("object", element);
    }
  );
  ...
  const isPrincipleElement = createPredicate(
    ({ hasBasicElementProps, isElementType, primitiveEq }) => {
      return (element) => element instanceof Principle || hasBasicElementProps(element) && isElementType("principle", element) && primitiveEq("object", element);
    }
  );
  const isRequirementElement = createPredicate(
    ({ hasBasicElementProps, isElementType, primitiveEq }) => {
      return (element) => element instanceof Requirement || hasBasicElementProps(element) && isElementType("requirement", element) && primitiveEq("object", element);
    }
  );
  const isRequirementLevelElement = createPredicate(
    ({ hasBasicElementProps, isElementType, primitiveEq }) => {
      return (element) => element instanceof RequirementLevel || hasBasicElementProps(element) && isElementType("requirementLevel", element) && primitiveEq("string", element);
    }
  );
  const isScenarioElement = createPredicate(
    ({ hasBasicElementProps, isElementType, primitiveEq }) => {
      return (element) => element instanceof Scenario || hasBasicElementProps(element) && isElementType("scenario", element) && primitiveEq("object", element);
    }
  );
  const isStandardElement = createPredicate(
    ({ hasBasicElementProps, isElementType, primitiveEq }) => {
      return (element) => element instanceof Standard || hasBasicElementProps(element) && isElementType("standard", element) && primitiveEq("object", element);
    }
  );
  const isStandardIdentifierElement = createPredicate(
    ({ hasBasicElementProps, isElementType, primitiveEq }) => {
      return (element) => element instanceof StandardIdentifier || hasBasicElementProps(element) && isElementType("standardIdentifier", element) && primitiveEq("array", element);
    }
  );
  ...
  class SpecificationVisitor extends Visitor {
    constructor(_a) {
      var _b = _a, { specObj } = _b, rest = __objRest$1(_b, ["specObj"]);
      super(__spreadValues$2({}, rest));
      __publicField$1(this, "specObj");
      __publicField$1(this, "passingOptionsNames", ["specObj"]);
    ...
    retrieveVisitorInstance(specPath, options = {}) {
      ...
      const visitorOpts = __spreadValues$2(__spreadValues$2({}, passingOpts), options);
  ...
  class FixedFieldsVisitor extends SpecificationVisitor {
    constructor(_a) {
      var _b = _a, { specPath, ignoredFields } = _b, rest = __objRest(_b, ["specPath", "ignoredFields"]);
  ...
  exports.MainElement = Main;
  exports.PrincipleElement = Principle;
  exports.RequirementElement = Requirement;
  exports.RequirementLevelElement = RequirementLevel;
  exports.ScenarioElement = Scenario;
  exports.SpecificationVisitor = SpecificationVisitor;
  exports.StandardElement = Standard;
  exports.StandardIdentifierElement = StandardIdentifier;
  exports.Visitor = Visitor;
  exports.default = apiDesignSystems;
  ...
  exports.refractPluginOpenApi3_1StandardIdentifierAccessors = plugin;
  exports.refractPluginOpenApi3_1StandardIdentifierSelectors = plugin$1;
  exports.validateOpenAPI3_1 = validate;
